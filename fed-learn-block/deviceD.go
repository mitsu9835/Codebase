...
func main() {
    ...
    if err != nil {
        log.Fatalf("Device D: open train.csv: %v", err)
    }
    defer f.Close()
    ...
    if err != nil {
        log.Fatalf("Device D: read header: %v", err)
    }
    n := len(header) - 1
    fmt.Printf("Device D: Detected %d features (excluding target)\n", n)
    ...
    for {
        ...
        if err != nil {
            log.Fatalf("Device D: read record: %v", err)
        }
        records = append(records, rec)
    }
    m := len(records)
    fmt.Printf("Device D: Total examples: %d\n", m)
    ...
    for i, rec := range records {
        ...
        for j := 0; j < n; j++ {
            ...
            if err != nil {
                log.Fatalf("Device D: parse feature %q: %v", rec[j], err)
            }
            X[i][j] = v
        }
        tv, err := strconv.ParseFloat(rec[n], 64)
        if err != nil {
            log.Fatalf("Device D: parse target %q: %v", rec[n], err)
        }
        y[i] = tv
    }
    ...
    fmt.Printf("Device D: Initialized theta: %v\n", theta)
    ...
    fmt.Printf("Device D: Dot product for example #1: %f\n", dot)

    // 7) Compute local gradient
    grad := computeLocalGradient(X, y, theta)
    fmt.Printf("Device D: Local gradient: %v\n", grad)
    ...
    if err != nil {
        log.Fatalf("Device D: read contract address: %v", err)
    }
    addr := common.HexToAddress(strings.TrimSpace(string(addrBytes)))
    fmt.Printf("Device D: Using Store at %s\n", addr.Hex())

    client, err := ethclient.Dial("http://127.0.0.1:8545")
    if err != nil {
        log.Fatalf("Device D: dial node: %v", err)
    }
    store, err := contracts.NewContracts(addr, client)
    if err != nil {
        log.Fatalf("Device D: bind contract: %v", err)
    }

    // 9) Fetch one noise sample per feature
    noises := make([]float64, n)
    for j := 0; j < n; j++ {
        nb, err := store.GenerateNoise(&bind.CallOpts{Context: context.Background()})
        if err != nil {
            log.Fatalf("Device D: generate_noise[%d]: %v", j, err)
        }
        noises[j] = float64(nb.Int64())
    }
    fmt.Printf("Device D: Noise vector η: %v\n", noises)

    // 10) Mask the gradient
    masked := functions.MaskGradient(grad, noises)
    fmt.Printf("Device D: Masked gradient: %v\n", masked)

    // 11) Compute BLS commitments
    g1 := bls.NewG1()
    ...
    if err := os.MkdirAll(workspace, 0755); err != nil {
        log.Fatalf("Device D: mkdir %s: %v", workspace, err)
    }
    _ = os.Remove(deviceFile) // clean any old file

    // 13) Write device_output.json
    out := struct {
    ...
    if err != nil {
        log.Fatalf("Device D: marshal JSON: %v", err)
    }
    if err := ioutil.WriteFile(deviceFile, data, 0644); err != nil {
        log.Fatalf("Device D: write %s: %v", deviceFile, err)
    }
    fmt.Printf("Device D: Wrote %s\n", deviceFile)

    // 14) Wait for fog2 signature
    fmt.Printf("Device D: Waiting for %s …\n", sigFile)
    for {
    ...
    // 15) Load Fog2’s public key
    pubData, err := ioutil.ReadFile(pubFile)
    if err != nil {
        log.Fatalf("Device D: read %s: %v", pubFile, err)
    }
    var pubJSON struct{ X, Y string }
    if err := json.Unmarshal(pubData, &pubJSON); err != nil {
        log.Fatalf("Device D: unmarshal %s: %v", pubFile, err)
    }
    ...
    // 16) Load Fog2’s signature
    sigData, err := ioutil.ReadFile(sigFile)
    if err != nil {
        log.Fatalf("Device D: read %s: %v", sigFile, err)
    }
    var sigJSON struct{ R, S string }
    if err := json.Unmarshal(sigData, &sigJSON); err != nil {
        log.Fatalf("Device D: unmarshal %s: %v", sigFile, err)
    }
    ...
    // 17) Verify Fog2’s signature on commitG
    if functions.VerifySignatureECDSA(pubKey, cgBytes, r, s) {
        fmt.Println("Device D: Fog signature valid ✅")
    } else {
        fmt.Println("Device D: Fog signature invalid ❌")
    }
}
