...
func main() {
	...
	n := len(header) - 1
	fmt.Printf("Device B: Detected %d features (excluding target)\n", n)
	...
	m := len(records)
	fmt.Printf("Device B: Total examples: %d\n", m)
	...
	fmt.Printf("Device B: Initialized theta: %v\n", theta)

	// 6) Dot product first example
	dot := 0.0
	for j := 0; j < n; j++ {
		dot += theta[j] * X[0][j]
	}
	fmt.Printf("Device B: Dot product for example #1: %f\n", dot)

	// 7) Local gradient
	grad := computeLocalGradient(X, y, theta)
	fmt.Printf("Device B: Local gradient: %v\n", grad)
	...
	fmt.Printf("Device B: Using Store at %s\n", addr.Hex())
	...
	fmt.Printf("Device B: Noise vector η: %v\n", noises)

	// 9) Mask gradient
	masked := functions.MaskGradient(grad, noises)
	fmt.Printf("Device B: Masked gradient: %v\n", masked)
	...
	// 11) Prep workspace & clear stale files
	...
	fmt.Printf("Device B: Wrote %s\n", deviceFile)

	// 13) Wait for Fog’s pubkey+signature
	fmt.Printf("Device B: Waiting for %s …\n", sigFile)
	for {
	...
	// 16) Verify Fog’s signature over this round’s commitG
	if functions.VerifySignatureECDSA(pubKey, cgBytes, r, s) {
		fmt.Println("Device B: Fog signature valid ✅")
	} else {
		fmt.Println("Device B: Fog signature invalid ❌")
	}
}
