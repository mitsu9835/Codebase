...
func main() {
    ...
    fogFile    := filepath.Join(workspace, "fog2_output.json")
    pubFile    := filepath.Join(workspace, "fog2_pubkey.json")
    ...
    for {
        ...
        fmt.Printf("FOG2: detected %s, loading…\n", deviceFile)
        ...
        if !g1.Equal(sum, commitM) {
            log.Fatal("FOG2: Commitment verification FAILED")
        }
        fmt.Println("FOG2: Commitment verification succeeded")

        // 7) Generate ECDSA keypair for Fog and publish the pubkey
        ...
        // 8) Sign the compressed commitG and write fog2_output.json
        ...
        fmt.Printf("FOG2: Wrote %s and %s\n", fogFile, pubFile)

        // 9) Remove the device file so the loop can handle the next one
        if err := os.Remove(deviceFile); err != nil {
            log.Fatalf("remove %s: %v", deviceFile, err)
        }
        // Loop back for the next device (C or D)
    }
}
